...
class GhostController extends ChangeNotifier {
  ...
  /// Установить текст (например черновик). Не влияет на транспорт.
  void setText(String text) {
    _chars.clear();
    _chars.addAll(text.split(''));
  ...
  /// Лёгкий no-op rebuild после долгого idle (OEM/Samsung/Huawei): слой жестов иногда «залипает».
  /// Не меняет текст; только [notifyListeners] с троттлингом.
  int _keepAliveSeq = 0;
  DateTime? _lastKeepAlive;

  void touchKeepAlive({Duration minInterval = const Duration(seconds: 45)}) {
    final now = DateTime.now();
    if (_lastKeepAlive != null &&
        now.difference(_lastKeepAlive!) < minInterval) {
      return;
    }
    _lastKeepAlive = now;
    _keepAliveSeq = (_keepAliveSeq + 1) & 0xffff;
    notifyListeners();
  }
