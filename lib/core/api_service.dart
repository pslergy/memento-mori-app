...
import 'internet/tunnel_config.dart' show kDefaultTunnelDonorHost;
...
class ApiService {
  ...
  Future<dynamic> _sendDirectHttp(
    ...
    // DPI-T01: для 4xx/5xx тело ответа уже получено — передаём размер в DpiDetector через logFailure.
    var failureResponseBytes = 0;
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        _connectionLogger.logSuccess(config: config);
        return response.body.isEmpty ? {} : jsonDecode(response.body);
      } else {
        failureResponseBytes = response.bodyBytes.length;
      ...
      _connectionLogger.logFailure(
        ...
        error: e,
        elapsed: elapsed,
        bytesTransferred: failureResponseBytes,
      );
      // DPI-T02: не крутить backendChannels при HTTP-ошибке с телом, TLS-handshake DPI и т.п.
      if (DpiBackendChannelPolicy.shouldRecordBackendChannelFailure(
            error: e,
            httpFailureBodyBytes: failureResponseBytes,
            elapsed: elapsed,
          )) {
  ...
  Future<dynamic> _sendViaMesh(
    ...
    try {
      ...
      final response = await locator<MeshCoreEngine>().sendThroughMesh(
          '/api$endpoint',
          method,
          headers,
