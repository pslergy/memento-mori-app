...
class UplinkCandidate {
  ...
  /// Когда узел выдал/обновил Wi‑Fi креденшал (BLE или gossip). Для проверки TTL «в воздухе».
  final DateTime? wifiCredentialIssuedAt;

  /// Срок жизни креденшала с [wifiCredentialIssuedAt] (сек). По умолчанию 2 ч.
  static const int defaultWifiCredentialTtlSec = 7200;
  final int wifiCredentialTtlSec;

  /// Креденшал ещё можно использовать для Wi‑Fi Direct connect (не истёк по локальным часам).
  bool get hasFreshWifiCredential {
    if (wifiDirectPassphrase == null || wifiDirectPassphrase!.isEmpty) return false;
    if (wifiCredentialIssuedAt == null) return true;
    final expires = wifiCredentialIssuedAt!.add(Duration(seconds: wifiCredentialTtlSec));
  ...
  /// [PREDICTIVE DISCOVERY] Last BLE RSSI (dBm). Used to decide Wi-Fi discovery timing: >-70 immediate, -85..-70 delayed, <=-85 skip.
  final int? rssi;
  ...
  UplinkCandidate({
    ...
    this.wifiCredentialIssuedAt,
    this.wifiCredentialTtlSec = defaultWifiCredentialTtlSec,
  ...
  UplinkCandidate copyWith({
    ...
    DateTime? wifiCredentialIssuedAt,
    int? wifiCredentialTtlSec,
    ...
    return UplinkCandidate(
      ...
      wifiCredentialIssuedAt: wifiCredentialIssuedAt ?? this.wifiCredentialIssuedAt,
      wifiCredentialTtlSec: wifiCredentialTtlSec ?? this.wifiCredentialTtlSec,
  ...
  factory UplinkCandidate.fromBleScan({
    ...
    final pass = wifiDirectPassphrase ?? parsedWifiPassphrase;
    return UplinkCandidate(
      id: candidateId,
      mac: mac,
      ...
      wifiDirectPassphrase: pass,
      wifiDirectNetworkName: wifiDirectNetworkName ?? parsedWifiNetworkName,
      wifiCredentialIssuedAt: pass != null && pass.isNotEmpty ? DateTime.now() : null,
      wifiCredentialTtlSec: defaultWifiCredentialTtlSec,
