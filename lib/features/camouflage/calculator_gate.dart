...
class _CalculatorGateState extends State<CalculatorGate> {
  ...
  Future<void> _handleAccess() async {
    if (_expression == '9111') {
      if (kDebugMode) debugPrint('[GATE] Silent Wipe PIN triggered (hard REAL purge).');
      await PanicService.hardPurgeRealData(context);
      return;
    }
    ...
    // До await: context после gap линтер/рантайм.
    final scopeLookupContext = context;

    final currentMode = await getGateMode();
    if (currentMode != resolved) {
      if (currentMode == AppMode.DECOY) stopDecoyBeaconFeed();
      final scope = scopeLookupContext.findAncestorStateOfType<DiRestartScopeState>() ??
          DiRestartScopeState.maybeInstance;
      if (scope != null && scope.mounted) {
        await scope.rebindMode(resolved);
      } else {
        await teardownSession();
        locator.reset();
        setupCoreLocator(resolved);
        setupSessionLocator(resolved);
