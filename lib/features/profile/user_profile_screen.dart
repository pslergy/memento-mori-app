...
class _UserProfileScreenState extends State<UserProfileScreen> {
  ...
  bool _isMe = false;
  String? _friendStatus; // 'accepted' | 'pending' | 'pending_outgoing' | null
  String _displayName = '';
  ...
  Future<void> _addFriend() async {
    try {
      ...
      await _mesh.sendFriendRequest(widget.userId,
          message: 'Привет! Добавь меня в друзья',
          peerUsername: widget.displayName);
      if (mounted) {
  ...
  Future<void> _removeFriend() async {
    try {
      ...
      await _db.removeFriendCompletely(widget.userId,
          ownerUserId: _api.currentUserId);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
                  ...
                  if (_isMe)
                    ...
                    if (_friendStatus == 'accepted')
                      OutlinedButton.icon(
                        ...
                        _friendStatus == 'pending_outgoing')
                      Padding(
