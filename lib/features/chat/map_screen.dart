...
/// Только счётчики: outbox, SOS за 24 ч. Обновляется при изменении MeshCoreEngine (отправка/удаление из outbox).
...
class _MapScreenState extends State<MapScreen> {
  ...
  int _sos24h = 0;
  bool _loading = true;
  DateTime? _lastLoadFromListener;
  static const _minIntervalFromListener = Duration(seconds: 2);

  void _onMeshOrOutboxChanged() {
    if (!mounted) return;
    // Дебаунс: при частых циклах MeshCoreEngine не дергаем загрузку чаще раз в 2 сек
    final now = DateTime.now();
    if (_lastLoadFromListener != null &&
        now.difference(_lastLoadFromListener!) < _minIntervalFromListener) {
      return;
    }
    _lastLoadFromListener = now;
  ...
  void initState() {
    ...
    if (locator.isRegistered<MeshCoreEngine>()) {
      locator<MeshCoreEngine>().addListener(_onMeshOrOutboxChanged);
    }
  }
  ...
  void dispose() {
    if (locator.isRegistered<MeshCoreEngine>()) {
      locator<MeshCoreEngine>().removeListener(_onMeshOrOutboxChanged);
    }
    super.dispose();
  }
  ...
  Future<void> _load({bool showLoading = true}) async {
    ...
    try {
      ...
      final newOutbox = pending.length;
      final newSos = sos;
      if (!mounted) return;
      final changed = newOutbox != _outboxPending || newSos != _sos24h || _loading;
      if (changed) {
        setState(() {
          _outboxPending = newOutbox;
          _sos24h = newSos;
          _loading = false;
        });
      }
