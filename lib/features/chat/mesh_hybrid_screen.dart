...
class _MeshHybridScreenState extends State<MeshHybridScreen>
  ...
  void initState() {
    super.initState();
    if (!locator.isRegistered<MeshCoreEngine>() && isCoreReady) {
    ...
    _sonarService = locator.isRegistered<UltrasonicService>()
        ...
        : null;
    if (_meshService == null && locator.isRegistered<MeshCoreEngine>()) {
      _meshService = locator<MeshCoreEngine>();
    }
    _radarController =
  ...
  Widget _buildMainContent(MeshCoreEngine mesh) {
    return Column(
      children: [
  ...
  Future<void> _copyAllLogs() async {
    ...
    if (mesh == null) return;
    try {
      // Получаем все логи из MeshCoreEngine (не только видимые)
              ...
              child: ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  if (locator.isRegistered<MeshCoreEngine>()) {
                    await locator<MeshCoreEngine>().connectToNode(node.id);
                  }
                }, // Вот здесь была пропущена запятая
                child: context.watch<MeshCoreEngine>().isTransferring
                    ? const SizedBox(
                        width: 12,
                        height: 12,
