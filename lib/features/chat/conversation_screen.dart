...
class ChatMessage {
  ...
  ChatMessage copyWith({String? status, String? crdtId, bool? isLocal, String? replyToId, String? replyPreview}) {
    return ChatMessage(
      id: id,
...
class _ConversationScreenState extends State<ConversationScreen>
  ...
  Future<void> _reloadMessagesForReveal() async {
    ...
    final history = (BeaconCountryHelper.isBeaconChat(_chatId) || _chatId == 'BEACON_NEARBY')
        ? await db.getMessages(_chatId!, limit: 500, substituteContent: false)
        : await db.getMessages(_chatId!, substituteContent: false);
    if (mounted) {
  ...
  Future<void> _initializeChat() async {
    ...
    final history = (BeaconCountryHelper.isBeaconChat(_chatId) || _chatId == 'BEACON_NEARBY')
        ? await db.getMessages(_chatId!, limit: 500, substituteContent: substitute)
        : await db.getMessages(_chatId!, substituteContent: substitute);
    setState(() {
      _messages = history;
  ...
  void _listenToChannels() {
    ...
    if (locator.isRegistered<MeshCoreEngine>()) {
      ...
      _meshSubscription = locator<MeshCoreEngine>().messageStream.listen((packet) {
