...
param(
  ...
  # Не задавайте по умолчанию через $PSScriptRoot — при `powershell -File ...` он бывает пустым.
  [string] $RepoRoot = ''
)
...
if ([string]::IsNullOrWhiteSpace($RepoRoot)) {
  ...
  if ([string]::IsNullOrWhiteSpace($scriptDir)) {
    throw "Cannot resolve script directory. Run from repo: .\scripts\export_public_mirror.ps1 ... or pass -RepoRoot D:\memento_mori_app"
  }
...
$excludeDirs = @('.git', '.dart_tool', 'build', '.idea', 'ios/Pods', 'node_modules', '_public_export', 'public_mirror_out', 'sdk', 'sdk_products')
...
# На всякий случай: расширение .vmd (если появятся заметки/экспорты)
$vmdFiles = Get-ChildItem -LiteralPath $dest -Recurse -File -Filter "*.vmd" -ErrorAction SilentlyContinue
if ($vmdFiles) {
  $vmdFiles | Remove-Item -Force
  Write-Host "  removed: all *.vmd ($($vmdFiles.Count) file(s))"
}
